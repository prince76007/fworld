...
global $server_file_path;
    ...
    if(isset($searchValue) && $searchValue!=""){
     $where_sql.=' AND (uwr_id = "'.$searchValue.'" OR customer_support_fb_id LIKE "%'.$searchValue.'"  )';
    }
    $sql=" SELECT  *  FROM `customer_support` $join_sql  $where_sql  ORDER BY  customer_support_id DESC ";
   ...
   $totalFiltered=count($all__request_data_total);
    ...
    $data_list=mysqli_fetch_all($query, MYSQLI_ASSOC);
     ...
     foreach ($data_list as $value) {
       $id=$value['customer_support_id'];
      $customer_support_status=$value['customer_support_status'];
      $uwr_is_verified=$value['uwr_is_verified'];
      $userid=$value['customer_support_fb_id'];    
    ...
    $details=""; 
      if($value['customer_support_description']){
      $details.="<p> <textarea>".$value['customer_support_description' ]."</textarea></p>";     
      }

        $edit_url='<a href="'.$edit_url1.'" class="btn btn-default"><i class="fa fa-eye" title="view order"></i>View</a>&nbsp; ';
        $remove_url=' <a class="remove_customer_support_id btn btn-sm btn-danger btn-sm"   data_id="'.$id.'"  >Delete</a> '; 
        
        $action=  $remove_url;
      
  		 $icon= '-';
		 if($value['customer_support_attechment_file']){
  		    $icon='<a href="'.$value['customer_support_attechment_file'].'" target="_blank"><img src="'.$value['customer_support_attechment_file'].'" width="80" height="80"></a>';
         }

      $single=array(
                      $id ,//.'-'. $value['fb_id'] ,
                      $user_data,
                      $value['customer_support_type'],
                      $details,
                      $icon,
                      $value['customer_support_contact_type'],
                      $value['customer_support_contact'],
                      $value['customer_support_cr_date'],
                      //$uwr_is_verified,
                      //$uwr_pay_status,
                      $action,
	               );
