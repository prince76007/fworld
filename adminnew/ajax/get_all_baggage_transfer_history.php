...
function user_baggage_history_list_ajax( )
  ...
  global $mysqli;
    ...
    $join_sql=" JOIN users as senderuser on  senderuser.fb_id=ubh.ubh_sender_fb_id  JOIN users as receiveruser on receiveruser.fb_id=ubh.ubh_fb_id  ";
    ...
    if($form_date1!="" && $end_date1!=""){
      $where_sql.=' AND  (ubh.ubh_cr_date BETWEEN "'.$form_date1.'%" and "'.$end_date1.'%") ';
    }


    if(isset($searchValue) && $searchValue!=""){
     $where_sql.=' AND (ubh.ubh_fb_id = "'.$searchValue.'" OR  ubh.ubh_sender_fb_id = "'.$searchValue.'" )';
   ...
   $totalFiltered=count($all__request_data_total);
    ...
    $data_list=mysqli_fetch_all($query, MYSQLI_ASSOC);
      ...
      $is_certified=" - ";
           $sting=" ".$value['ubh_gift_qty'].'*'.$value['ubh_gift_diamond'].'='.$value['ubh_gift_total_diamond'];       

      $senderuser_details='<p>'.$value['senderuser_first_name'].' '.$value['senderuser_last_name'].'</p>';
      $senderuser_details.='<p>'.$value['ubh_sender_fb_id']  .'</p>';
     
      $receiveruser_details='<p>'.$value['receiveruser_first_name'].' '.$value['receiveruser_last_name'].'</p>';
      $receiveruser_details.='<p>'.$value['ubh_fb_id'].'</p>';
                      
      $single=array(
                      $value['ubh_id'] ,//.'-'. $value['fb_id'] ,
                      $senderuser_details,
                      $receiveruser_details,
                      $value['ubh_gift_title'],
                      $sting,
                     // $value['dth_for'],
                      $value['ubh_cr_date'],
                      $status,
