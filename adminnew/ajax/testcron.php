...
$current_datetime =date('Y-m-d H:i:s', strtotime($current_datetime1.' - 3 minute'));
...
echo $sel_sql=" Select ulrd_id,ulrd_isuser_pdatetime From user_live_room_details  where ulrd_isuser_pdatetime < '$current_datetime'  and ulrd_isuser_pdatetime!=''  ORDER by ulrd_id DESC ";
  ...
  $res=mysqli_query($mysqli,$sel_sql)or die(mysqli_error($mysqli));
   $da=mysqli_fetch_all($res, MYSQLI_ASSOC); 
		       
print_r($da);
echo $update_sql=" update user_live_room_details set ulrd_status=4 , ulrd_isuser_online=1 where ulrd_isuser_pdatetime < '$current_datetime' and   ulrd_isuser_pdatetime!=''  and ulrd_status=0 and ulrd_isuser_online=0 ORDER by ulrd_id DESC ";
