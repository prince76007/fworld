...
function UPDATE_add_silvercoin_user($fb_id ,$add_silver_coin)
	...
	function get_user_on_hoster_live_room($fb_id){
		include("config.php");
    	//USER Host LIVE ROOM     
        $qrry_get="select ULRD.*,u.username,u.profile_pic,u.first_name,u.last_name from  user_live_room_details as ULRD join users as u on u.fb_id=ULRD.ulrd_fb_id WHERE ULRD.ulrd_fb_id = '$fb_id' and 	ULRD.ulrd_status='0' ";
		$res=mysqli_query($conn,$qrry_get) or die(mysqli_error($conn));
    ...
    */
        //USER JOIN LIVE ROOM     
        $qrry_get="select UJLRD.*,ULRD.*,u.username,u.profile_pic,u.first_name,u.last_name from user_join_live_room_details  as UJLRD JOIN user_live_room_details as ULRD ON ULRD.ulrd_id=UJLRD.ujlrd_ulrd_id  join users as u on u.fb_id=ULRD.ulrd_fb_id WHERE UJLRD.ujlrd_fb_id = '$fb_id' and 	UJLRD.ujlrd_status='0' and ULRD.ulrd_status='0'";
		$res=mysqli_query($conn,$qrry_get) or die(mysqli_error($conn));
 		$data=mysqli_fetch_assoc($res); 
		//print_r($giftdata);
		if($data){
...
//10_2_2021
    ...
    function user_top_fans($fb_id,$limit ){
	          ...
	          $query=mysqli_query($conn,"select *,sum(diamonds) as topdiamonds from view_top_fan as vtf   where vtf.reciever_id='".$fb_id."' group by vtf.sender_id  order by sum(diamonds) DESC limit  $limit ");
    		while($srow=mysqli_fetch_assoc($query))
    		{
                

    		    $sender_id=$srow['sender_id'];
    ...
    function contribution_user_data($fb_id,$limit ){
			...
			$query=mysqli_query($conn,"select *,sum(diamonds) as topdiamonds from view_contributer where sender_id='".$fb_id."'  group by reciever_id  order by sum(diamonds) DESC limit $limit");
	...
	{   
		...
		$event_json = json_decode($input,true);
	        ...
	        if($before_user_total_diamondd<$edtb_diamond){
				$msg_out="You can not exchange diamond to beans ";
			    $output=array( "code" => "201", "msg" => $msg_out ,  "data"=> "");
				print_r(json_encode($output, true)); 
				exit();
			}
	        ...
	        $DATE_TIME=custom_current_date_time();
	         ...
	         $qrry_get='INSERT INTO `user_exchange_diamond_to_beans`( `uedtb_fb_id`, `uedtb_request_diamond`, `uedtb_request_beans`, `uedtb_has_diamond`, `uedtb_has_beans`, `uedtb_cr_date`)  VALUES ("'.$fb_id.'","'.$edtb_diamond.'","'.$edtb_beans.'","'.$before_user_total_diamondd.'","'.$before_user_total_beans.'","'.$DATE_TIME.'")';
	...
	{   
		...
		$event_json = json_decode($input,true);
			...
			if($last_insertid){
				...
				$output=array( "code" => "200", "msg" => $msg_out ,"data" => $res , "level_up_response" => $level_up_response );
			}else{
	...
	{   
	    ...
	    */
	    $query          = mysqli_query($conn," select * from gift WHERE gift_status='Active' ");
	    $gift          =  array();
...
function user_create_live_room_detail(){
		...
		$event_json = json_decode($input,true);
			...
			$insert_sql='INSERT INTO `user_live_room_details`( `ulrd_room_id`, `ulrd_fb_id`, `ulrd_stream_id`,`ulrd_stream_title`,`ulrd_icon`,`ulrd_stream_label`,`ulrd_cr_date`) VALUES ("'.$ulrd_room_id.'","'.$fb_id.'","'.$ulrd_stream_id.'","'.$ulrd_stream_title.'","'.$ulrd_icon.'","'.$ulrd_stream_label.'","'.$DATE_TIME.'")';
	...
	function  hoster_user_change_status_of_live_room()
	{
		...
		$event_json = json_decode($input,true);
			...
			$update_sql='Update `user_live_room_details` set ulrd_status="'.$ulrd_status.'" 
     	        ...
     	        where ulrd_id="'.$ulrd_id.'" and ulrd_fb_id="'.$fb_id.'"';
...
function user_join_live_room_detail(){
    	...
    	$sql="SELECT *  FROM `user_live_room_details` where ulrd_id=$ulrd_id and ulrd_status in(0) ";
        ...
        $sql="SELECT ujlrd_id  FROM `user_join_live_room_details` where ujlrd_ulrd_id=$ulrd_id and ujlrd_status in(0,3) and ujlrd_fb_id =$fb_id";
...
//echo "UPDATE users SET  total_diamondd = total_diamondd + '$diamond' , total_earning = total_earning + '$after_5prt' WHERE fb_id = '$r_fb_id' ";die;
	...
	{   
        ...
        $ch      = curl_init('http://msg.pwasms.com/app/smsapi/index.php?key=45E8C845640108&campaign=0&routeid=9&type=text&contacts='.$mobile.'&senderid=PWASMS&msg='.$message.'');
...
//REKHA MAM 
	function get_contribute()
	{
		...
		if(isset($event_json['fb_id']))
		{
		    ...
		    $query=mysqli_query($conn,"select *,sum(diamonds) as topdiamonds from view_contributer where sender_id='".$fb_id."'  group by reciever_id  order by sum(diamonds) DESC");
...
function get_topfans()
	{
	   //echo "Ca";
		...
		if(isset($event_json['fb_id']))
		{
		    ...
		    $query2=mysqli_query($conn,"select *,sum(diamonds) as topdiamonds from view_top_fan as vtf   where vtf.reciever_id='".$fb_id."' group by sender_id  order by sum(diamonds) DESC limit 1"); 
               $singlerow=mysqli_fetch_assoc($query2);
             // print_r($rows);
		       		
		       	$sender_id=$singlerow['sender_id'];
		       	$sql="SELECT * FROM users where `fb_id`='$sender_id'";
				$query=mysqli_query($conn,$sql);
				$row=mysqli_fetch_assoc($query);
			    ...
			    $row['follow_Status']=get_follow_users_status($row['fb_id'], $fb_id);
		     	...
		     	$row['famliy_tag_icon']='/custom_uploads/family_tag_icon/familytag.png'; 	
		       	   	$sresult = array_merge($singlerow,$row);
		            $array_out = $sresult; 
		      // $arr=$singlerow;
		       /*array(
             ...
             $query=mysqli_query($conn,"select *,sum(diamonds) as topdiamonds from view_top_fan as vtf   where vtf.reciever_id='".$fb_id."' group by vtf.sender_id  order by sum(diamonds) DESC");
    		while($srow=mysqli_fetch_assoc($query))
    		{
...
//5_2_2021
	function TOP_STAR_mode_Daily($fb_id=null,$ranking_mode , $ranking_type){
	        include("config.php");



				$sqli="SELECT fb_id,mobile,first_name,last_name,profile_pic,user_wear_badge,total_diamondd,total_earning,total_expo,total_beans,total_silver_coin,vip_status FROM `users` limit 20";
				$result=mysqli_query($conn,$sqli) or die(mysqli_error($conn));
		        $userdata=mysqli_fetch_all($result, MYSQLI_ASSOC); 
			 ...
			 return	$output=array( "code" => "500", "msg" => $msg_out , "ranking_mode"=>$ranking_mode , "ranking_type" =>$ranking_type, "data"=> ""  );
				}
			...
			$fb_id=$event_json['fb_id'];
		  	...
		  	if($ranking_type =='TOP_STAR'){
		  		...
		  		switch ($ranking_mode) {
		  			case 'Daily':
		  				# code...
		  			 $data_out=TOP_STAR_mode_Daily($fb_id ,$ranking_mode , $ranking_type); 
		  			 break;
